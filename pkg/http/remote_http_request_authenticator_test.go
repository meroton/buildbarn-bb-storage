...
func TestRemoteHttpRequestAuthenticator(t *testing.T) {
	...
	t.Run("BackendSuccess", func(t *testing.T) {
		...
		authenticator, err := bb_http.NewRemoteHTTPRequestAuthenticator(
			backend,
			[]string{
				"Authorization",
				"Other-Header",
	...
	t.Run("OnlyAcceptCanonicalHeaders", func(t *testing.T) {
		_, err := bb_http.NewRemoteHTTPRequestAuthenticator(
			backend,
